...
void WRLD::ParseChunks(WRLD& aSourceRecord, Map<uint8_t, uint32_t>& aParentToFormIdPrefix) noexcept
{
    aSourceRecord.IterateChunks(
        [&](ChunkId aChunkId, Buffer::Reader& aReader)
        {
            ...
            case ChunkId::WCTR_ID: m_centerCell = Chunks::WCTR(aReader); break;
            case ChunkId::CNAM_ID: aReader.ReadBytes(reinterpret_cast<uint8_t*>(&m_climateId), sizeof(m_climateId)); break;
            case ChunkId::DNAM_ID: m_landData = Chunks::DNAM(aReader); break;
